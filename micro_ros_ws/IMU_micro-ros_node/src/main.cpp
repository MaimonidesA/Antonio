...
//orientation
	...
	m = AngleAxisd(roll, 	    	Vector3d::UnitX())
	  ...
	  * AngleAxisd(yaw * M_PI / 180,Vector3d::UnitZ());
	q = m;
	ImuMsg.orientation.x = roll;
	ImuMsg.orientation.y = Pitch;
	ImuMsg.orientation.z = yaw * M_PI / 180;
	//ImuMsg.orientation.w = q.w();

    //ImuMsg.angular_velocity.x = gyro_roll * M_PI / 180;
	//ImuMsg.angular_velocity.y = Pitch * M_PI / 180;
	//ImuMsg.angular_velocity.z = yaw * M_PI / 180;
...
void core_1() {
   ...
   while (err != AK09918_ERR_OK)
   {
    ...
    sleep_ms(1000);
    calibrate_magnometer( 1000000, &Magnetic_offset_X, &Magnetic_offset_Y, &Magnetic_offset_Z);
    ...
    for (size_t i = 0; i < 4000; i++)
    {
        ...
        endTime = time_us_32();
        acceleration_Z = icm20600.getAccelerationZ() * -1;
    ...
    while (1)
    {
        ...
        //Pitch
        Accelerometer_Pitch = atan(acceleration_X / sqrt( acceleration_Z * acceleration_Z + acceleration_Y * acceleration_Y));
        gyro_Pitch = (icm20600.getGyroscopeY()+2) ;   
        ...
        gyro_yaw = icm20600.getGyroscopeZ() * Delta_Time;
        double Xheading = Magnetic_X * cos(Magnetic_pitch) + Magnetic_Y * sin(Magnetic_roll) * sin(Magnetic_pitch) + Magnetic_Z * cos(Magnetic_roll) * sin(Magnetic_pitch);
        double Yheading = Magnetic_Y * cos(Magnetic_roll) - Magnetic_Z * sin(Magnetic_pitch);
